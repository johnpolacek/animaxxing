...
- Replay on a settled resize: observe the container's inline size, ignore moves under 24px from the entered width, then pause the intro and fade the container out over 140ms, setting `waiting` on fade completion. Wait 1000ms of quiet after the last width change (including moves back inside the threshold), then kill the intro, restore the container opacity, remount the page subtree (a React `key`, a Svelte `{#key}`, a Vue `:key`, or a fresh render), and call `buildPageIntro` again. See [Resize](../motion-vocabulary.md#resize).

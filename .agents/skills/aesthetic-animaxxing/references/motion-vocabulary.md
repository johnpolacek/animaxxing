...
- **Fade at the start.** Once the width crosses the threshold, pause the entrance, signal `exiting` to stop ambient effects, and fade the route container to zero opacity in about 140ms. Set `waiting` when the fade completes and keep it hidden during the drag.
- **Debounce the end.** Wait one full second after the last actual width change. Once resizing starts, every width change resets the timer, including a return inside the original threshold; height-only observations do not. Fade only once per resize session. Cancel the pending restart on navigation or unmount, and restore the container opacity before its fresh entrance.
- **Stop what breaks mid-drag.** The wave pins each letter to a pixel width, so it is wrong the moment the headline reflows. Stop it and revert its split on the first width change; if no replay follows, start it again after a beat longer than the page's settle (for example, 1150ms). Particle fields re-measure through their own observers and need nothing.
